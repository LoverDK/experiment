...
The first subsection, \emph{Debiased Archive Estimation}, establishes that each archived experiment can contribute a valid effect estimate.  Proposition~\ref{prop:unbiased} states the core orthogonality conclusion: exact unbiasedness under known randomization and a second-order nuisance remainder when propensities are estimated.  The auxiliary variance calculation used by the composition analysis is separated as Lemma~\ref{lem:variance-envelope}.
...
\begin{proposition}[Debiased archive effect]
\label{prop:unbiased}
Suppose Assumption~\ref{ass:identified} holds and the nuisance estimates in
\eqref{eq:aipw-score} are cross-fitted, with
\(\widehat\pi_i\in[\pi_0,1-\pi_0]\).  Let \(I_{ik}\) be evaluation fold
\(k\) and \(\mathcal T_{ik}\) its nuisance-training data.  If the design or
logging propensity is known, then
\[
  \E\left\{
  ...
  \E(\widehat\tau_i)=\tau_i.
\]
If the propensity is estimated instead, orthogonality removes first-order
plug-in bias and yields
\[
...
Under the conditions of Proposition~\ref{prop:unbiased}, additionally suppose
\(\abs{Y_i}\le M\), \(\widehat\mu_{i,a}\in[-M,M]\), and the evaluation
scores are conditionally independent given a common external nuisance-training
sample.  For independent archive experiments and any \(\alpha\in\Delta_C\),
\[
  \Var\left(
    \sum_{j\in C}\alpha_j\widehat\tau_j
    \,\middle|\,
...
The appendix follows the logical order of the main results.  It first proves
the conditional unbiasedness and nuisance-error claim in
Proposition~\ref{prop:unbiased}, followed by the auxiliary variance envelope
in Lemma~\ref{lem:variance-envelope}.  It then verifies the interval
...
\end{aligned}
\end{equation}
This is the stated second-order remainder and completes the proof of
