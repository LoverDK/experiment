...
\item \textbf{Debiased archive estimators.}  We replace raw plug-in effects by Horvitz--Thompson or AIPW experiment-level estimates.  We prove exact unbiasedness under known randomization or logging propensities, give a product-bias formula under estimated propensities, and derive finite-sample variance and concentration bounds after clipping and overlap.
...
\label{eq:aipw-estimator}
  ...
  \right\}^2,
\end{equation}
where \(\widehat\eta_i^{(-\ell)}\) denotes a nuisance fit trained without unit \(\ell\)'s fold.  In a randomized experiment with known propensity, \(\widehat\pi_i=\pi_i\), and \eqref{eq:aipw-estimator} is exactly unbiased conditional on the nuisance-training folds.  If no covariates are used, it reduces to the Horvitz--Thompson difference-in-means estimator.

\subsection{Support Testing and Transport}

For a target \(e_\star\), the atlas first retrieves a candidate set \(C\) using semantic and metadata filters.  Retrieval is deliberately permissive.  Identification is performed by the next layer.  After removing candidates with \(\kappa(e_j,e_\star)=0\), the atlas solves
\begin{equation}
...
\State Remove every \(j\in C\) with \(\kappa(e_j,e_\star)=0\).
...
\If{support residual, nuisance remainder, and moderator sensitivity imply \(Rad(\widehat\alpha,\zeta)\le \delta_{\rm sup}\)}
  \State \Return \(\widehat\theta_{\widehat\alpha}=\sum_{j\in C}\widehat\alpha_j\widehat\tau_j\) and the honest interval in Corollary~\ref{cor:honest-ci}.
\Else
...
The first subsection, \emph{Debiased Archive Estimation}, establishes that each archived experiment can contribute a valid effect estimate.  Proposition~\ref{prop:unbiased} proves exact unbiasedness under known randomization, gives the second-order nuisance-bias bound when propensities are estimated, and derives the finite-sample variance control needed for later composition.

The second subsection, \emph{Finite-Sample Composition}, establishes the central transport certificate.  Lemma~\ref{lem:wellposed} shows that the intersection of compatible certified intervals is a well-defined interval whenever the certificates are mutually consistent.  Theorem~\ref{thm:composition} then bounds the error of a transported composition by separating causal-support error, local curvature, hidden-moderator sensitivity, nuisance bias, and statistical noise.  Corollary~\ref{cor:semantic-special} identifies the fully observed affine case in which the composition is exact apart from estimation noise.

The third subsection, \emph{Asymptotic Normality and Honest Confidence Intervals}, establishes the large-sample inferential behavior of an accepted composition.  Theorem~\ref{thm:asymptotic} proves asymptotic normality around the composed archive estimand and specifies when the approximation radius is small enough for inference about the target effect itself.  Corollary~\ref{cor:honest-ci} converts this result into a confidence interval that remains valid when approximation error is non-negligible by explicitly inflating the interval with the certified deterministic radius.
...
\begin{proposition}[Unbiasedness, nuisance remainder, and finite-sample variance]
\label{prop:unbiased}
Suppose Assumption~\ref{ass:identified} holds. Let the nuisance estimates in
\eqref{eq:aipw-score} be cross-fitted and uniformly clipped so that
\(\widehat\pi_i\in[\pi_0,1-\pi_0]\) and
\(\widehat\mu_{i,a}\in[-M,M]\). Then:
\begin{enumerate}[label=(\roman*),leftmargin=2em]
\item Let \(I_{ik}\) be evaluation fold \(k\), and let
\(\mathcal T_{ik}\) denote the data used to fit
\(\widehat\eta_i^{(-k)}\). If
\(\widehat\pi_i^{(-k)}=\pi_i\) is the known design or logging propensity,
then the fold estimator is conditionally unbiased:
\[
  \E\left\{
    \frac{1}{|I_{ik}|}\sum_{\ell\in I_{ik}}
    \phi_i(W_{i\ell};\widehat\eta_i^{(-k)})
    \,\middle|\,\mathcal T_{ik}
  \right\}
  =
  \tau_i.
\]
Consequently, averaging over folds and applying the tower property gives
\(\E(\widehat\tau_i)=\tau_i\). If all nuisance functions are trained on one
external sample independent of every evaluation observation, the stronger
full-sample conditional identity also holds:
\[
  \E\{\widehat\tau_i\mid
  \widehat\eta_i^{(-1)},\ldots,\widehat\eta_i^{(-n_i)}\}
  =
  \tau_i.
\]
In particular, the Horvitz--Thompson difference-in-means estimator is
exactly unbiased in a randomized experiment.

\item If \(\widehat\pi_i\) is estimated, the following conditional bound
holds fold by fold, with every fitted nuisance function interpreted as the
fit trained outside that fold:
\[
\begin{aligned}
  \abs{\E(\widehat\tau_{ik}-\tau_i\mid\mathcal T_{ik})}
  &\le
  \frac{1}{\pi_0}
  \norm{\widehat\pi_i^{(-k)}-\pi_i}_{2,i}
  \left\{
  \norm{\widehat\mu_{i,1}^{(-k)}-\mu_{i,1}}_{2,i}
  +
  \norm{\widehat\mu_{i,0}^{(-k)}-\mu_{i,0}}_{2,i}
  \right\}
  \defeq \mathfrak b_{ik}.
\end{aligned}
\]
Hence
\[
  \abs{\E(\widehat\tau_i-\tau_i)}
  \le
  \sum_{k=1}^K\frac{|I_{ik}|}{n_i}\E(\mathfrak b_{ik})
  \defeq
  \mathfrak b_i.
\]
Thus the first-order plug-in bias is removed by the orthogonal score.

\item Suppose additionally that \(\abs{Y_i}\le M\) almost surely and that
the evaluation scores are conditionally independent given a common
nuisance-training sample, as under independent external nuisance training.
Then
\[
  \Var(\widehat\tau_i\mid\widehat\eta_i)
  =
  \frac{1}{n_i}
  \Var_i\{\phi_i(W_i;\widehat\eta_i)\mid\widehat\eta_i\}
  \le
  \frac{C_M^2}{n_i},
  \qquad
  C_M=2M+\frac{2M}{\pi_0}.
\]
Consequently, for any \(\alpha\in\Delta_C\) and independent archive
experiments,
\[
  \Var\left(
    \sum_{j\in C}\alpha_j\widehat\tau_j
...
\]
For ordinary fold-specific cross-fitting, part (iii) is applied foldwise;
the archive-level finite-sample dependence is summarized by the uncertainty
certificate in Assumption~\ref{ass:noise}.
\end{enumerate}
\end{proposition}

The proposition has a simple implication.  The archive should not store raw coefficients when the original design permits a debiased certificate.  A biased plug-in estimate is first converted into an AIPW or Horvitz--Thompson object; any remaining bias is explicitly recorded as the second-order term \(\mathfrak b_i\).
...
\begin{corollary}[Semantic atlas as a special case]
\label{cor:semantic-special}
Suppose \(r(e)=m(e)\), \(R_{\rm hid}(\alpha)=0\), \(\mathfrak b_j=0\), \(\kappa(C,\star)=1\), and \(\mu\) is affine on \(\conv\{m_\star,m_j:j\in C\}\).  If \(m_\star=\sum_{j\in C}\alpha_jm_j\), then
\[
  \tau_\star=\sum_{j\in C}\alpha_j\tau_j.
\]
With estimated archive effects, the only remaining error in \(\sum_j\alpha_j\widehat\tau_j\) is the statistical term in \eqref{eq:composition-bound}.
\end{corollary}

The corollary clarifies the relationship to semantic-atlas methods.  They are not wrong in the favorable case.  They are incomplete as a general identification strategy because they assume away the very moderators and design incompatibilities that the atlas must diagnose.
...
\begin{theorem}[Asymptotic distribution of an accepted atlas composition]
...
and the product-rate bound in Proposition~\ref{prop:unbiased}. Assume the
...
\begin{corollary}[Honest atlas confidence interval]
\label{cor:honest-ci}
Under the conditions of Theorem~\ref{thm:asymptotic}, the interval
\begin{equation}
\label{eq:honest-ci}
  ...
  \right]
\end{equation}
has asymptotic coverage at least \(1-\zeta\) whenever the one-sided radius
...
A common objection is that a sufficiently strong embedding could include the moderator.  That objection is correct and is precisely why the theorem is useful.  If the representation is causally sufficient, the theorem does not apply and Corollary~\ref{cor:semantic-special} describes the favorable case.  The impossibility concerns representations that are many-to-one over moderators relevant to treatment effects.
...
The appendix follows the logical order of the main results.  The conditional
unbiasedness and nuisance-error calculations are proved in
Proposition~\ref{app:proof-prop-unbiased}; the interval construction is
checked in Lemma~\ref{app:proof-lem-wellposed}; and the finite-sample
composition certificate from Theorem~\ref{thm:composition} is established in
Theorem~\ref{app:proof-thm-composition}.  The special affine case is recorded
in Corollary~\ref{app:proof-cor-semantic}.  The normalized triangular-array
argument and the confidence-interval argument appear in
Theorems~\ref{app:proof-thm-asymptotic} and
\ref{app:proof-cor-honest-ci}, respectively.  Finally,
Theorem~\ref{app:proof-thm-impossibility} proves the boundary result,
Theorem~\ref{app:proof-thm-partial-id} proves partial identification,
Theorem~\ref{app:proof-thm-minimax} proves the minimax lower bound, and
Theorem~\ref{app:proof-thm-bridge} proves the bridge-design guarantee used in
Section~\ref{sec:theory}.

\subsection{Proof of Proposition~\ref{prop:unbiased}}
\label{app:proof-prop-unbiased}

\paragraph{Proof strategy.} We condition on the nuisance-training folds first.  The proof then evaluates one score at a time, derives its conditional mean, controls the second-order remainder, and finally establishes the variance bound.
...
in \eqref{eq:app-proof-010}, and finally obtain the variance envelope in
\eqref{eq:app-proof-012}.
...
For part (i), the precise claim needed for ordinary cross-fitting is the foldwise conditional identity 
\begin{equation}
...
\(\E\{\phi(W;\widehat\mu_0,\widehat\mu_1,\pi)\}=\tau\).  Taking expectations over each training sample and then forming the fold-size-weighted average gives the second line of \eqref{eq:app-proof-008}, so part (i) follows.  A conditional identity given all fitted nuisances requires the stronger external-training setup stated in the proposition; it is not needed for ordinary cross-fitting.  The Horvitz--Thompson estimator is obtained by taking
\(\widehat\mu_0=\widehat\mu_1=0\), so its unbiasedness follows as a special
case.

For part (ii), let \(\widehat\pi\) be a clipped estimated propensity.  Conditional on \(X\) and the nuisance training folds,
\begin{equation}
...
\end{aligned}
\end{equation}
This is the stated second-order remainder.  For part (iii), independent external nuisance training makes the evaluation scores conditionally independent and identically distributed, giving the variance reduction in \eqref{eq:app-proof-011}; the
clipped-score envelope in \eqref{eq:app-proof-012} then supplies the claimed
finite-sample variance bound.
...
\subsection{Proof of Corollary~\ref{cor:semantic-special}}
\label{app:proof-cor-semantic}

\paragraph{Proof strategy.} Under the stated special-case assumptions, the mechanism representation is exact and the effect surface is affine, so the composition identity follows by direct algebra.

This is precisely the zero-support-residual and zero-hidden-moderator case of
Theorem~\ref{thm:composition}; the exact identity to be shown is the main
paper's semantic-special conclusion in Corollary~\ref{cor:semantic-special}.

If \(r(e)=m(e)\), \(R_{\rm hid}(\alpha)=0\), and \(m_\star=\sum_j\alpha_jm_j\), then \(\rho_\star(\alpha)=0\).  If \(\mu\) is affine on the relevant convex hull, Jensen's identity for affine functions gives
\begin{equation}
\label{eq:app-proof-025}
\mu(m_\star)=\mu\left(\sum_j\alpha_jm_j\right)=\sum_j\alpha_j\mu(m_j).
\end{equation}
The affine cancellation is the identity in \eqref{eq:app-proof-025}.  Since
\(\tau_\star=\mu(m_\star)\) and \(\tau_j=\mu(m_j)\), it proves the exact
identity in Corollary~\ref{cor:semantic-special}.  Replacing \(\tau_j\) by
\(\widehat\tau_j\) introduces only the weighted estimation error controlled
in Theorem~\ref{thm:composition}.
...
\subsection{Proof of Corollary~\ref{cor:honest-ci}}
\label{app:proof-cor-honest-ci}

\paragraph{Proof strategy.} The confidence statement follows by combining a statistical event for the centered estimator with a one-sided event ensuring that the estimated approximation radius is not too small.
