...
本阶段检验拒绝行为和区间覆盖是否像证书所声明的那样工作。实验包含4个场景、
...
使用正确上界时，ATLAS 在名义场景的发布率为0.4300；隐藏半径异质时为0.0300；
...
故意低报平滑界的策略也发布所有点，但其已发布区间覆盖率在强失配下跌至0.8767，
在严重失配下进一步跌至0.7533。这就是本实验要展示的失效边界：数据生成机制仍然
