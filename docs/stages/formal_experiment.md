...
本阶段不是 Algorithm 1 的附加步骤，而是对统一入口第 2--7 行及消融方法做
多随机种子正式验证。它用正式的合并协议替代阶段4的单种子筛查。实验包含6个场景、3个独立基础
...
在名义场景中，完整 ATLAS 接受0.4633的 target，接受后的平均绝对误差为0.1109。
不拒绝版本的平均绝对误差为0.1387；语义强制组合、最近语义邻居和全局均值分别为
0.2241、0.4557和0.2816。

语义失配从0增加到0.25时，ATLAS 接受率从0.4633降到0.2833，接受后的平均绝对
误差从0.1109升到0.1581。隐藏半径为0.40时，所有 target 都被拒绝。样本量从100
增加到1000时，接受率从0.2300升到0.5000。

取消方差正则对名义场景的影响较小：接受率从0.4633变为0.4400，接受后的平均绝对
