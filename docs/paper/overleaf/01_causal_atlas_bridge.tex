...
Across the synthetic experiments, Causal ATLAS improves composition when causal support is informative, exposes a transparent risk--coverage frontier, and protects interval validity when its declared uncertainty bounds are correct.  When support is insufficient, it transitions to partial identification and avoids forced extrapolation; bridge experiments then shrink the resulting diameter.  NSW is consistent with these mechanisms as a reconstruction stress test, while supplying no causal ground truth for subgroup effects.
